...
key_dim = filters[-1]
output_layers = 2
latent_gaussian_shape = (2, 2, filters[-1])
...
def get_model_conv_alex(x_train_images, x_train_labels):
    ...
    x_latent_stddev = tf.keras.layers.Conv2D(filters=x.shape[-1],
                                             kernel_size=(1, 1),
                                             strides=(1, 1),
                                             padding="same",
...
def validate(model, x_test_images, standard_scaler, x_test_labels, i):
    ...
    if conv_bool and alex_bool:
        if i > mean_squared_error_epoch and i > root_mean_squared_error_epoch:
