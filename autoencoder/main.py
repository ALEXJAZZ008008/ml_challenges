...
num_heads = None
key_dim = None
latent_gaussian_shape = (4, 4, filters[-1])
output_layers = 2
...
root_mean_squared_error_epoch = 2
...
def preprocess_labels(x_train_labels, x_test_labels):
    ...
    x_train_labels = np.reshape(one_hot_encoder.fit_transform(np.reshape(x_train_labels, (-1, 1))),
                                (x_train_labels.shape[0], -1))
    x_test_labels = np.reshape(one_hot_encoder.transform(np.reshape(x_test_labels, (-1, 1))),
                               (x_test_labels.shape[0], -1))
...
def preprocess_input(x_train_images, x_test_images, x_train_labels, x_test_labels):
    ...
    return x_train_images, x_test_images, standard_scaler, x_train_labels, x_test_labels
...
def get_model_conv_alex(x_train_images, x_train_labels):
    ...
    x_latent_gaussian_input = tf.keras.Input(shape=latent_gaussian_shape)
    ...
    x = tf.keras.layers.Conv2D(filters=filters[0],
                               ...
                               padding="same",
                               dilation_rate=1,
                               kernel_initializer=tf.keras.initializers.he_uniform)(x_image_input)
    ...
    for i in range(output_layers):
        x_latent_mean = tf.keras.layers.Conv2D(filters=x_latent_mean.shape[-1],
                                               kernel_size=(3, 3),
                                               strides=(1, 1),
                                               padding="same",
                                               dilation_rate=1,
                                               kernel_initializer=tf.keras.initializers.he_uniform)(x_latent_mean)
    ...
    x_latent_mean = tf.keras.layers.Conv2D(filters=x_latent_mean.shape[-1],
                                           ...
                                           padding="same",
                                           dilation_rate=1,
                                           kernel_initializer=tf.keras.initializers.ones)(x_latent_mean)

    x_latent_stddev = x

    for i in range(output_layers):
        x_latent_stddev = tf.keras.layers.Conv2D(filters=x_latent_stddev.shape[-1],
                                                 kernel_size=(3, 3),
                                                 strides=(1, 1),
                                                 padding="same",
                                                 dilation_rate=1,
                                                 kernel_initializer=tf.keras.initializers.he_uniform)(x_latent_stddev)
    ...
    x_latent_stddev = tf.keras.layers.Conv2D(filters=x_latent_stddev.shape[-1],
                                             ...
                                             padding="same",
                                             dilation_rate=1,
                                             kernel_initializer=tf.keras.initializers.zeros)(x_latent_stddev)
    x_latent_stddev = tf.keras.layers.Lambda(tf.keras.activations.softplus)(x_latent_stddev)
    x_latent_stddev = tf.keras.layers.Lambda(lambda current_x: current_x + 0.3068528)(x_latent_stddev)

    # x = tf.keras.layers.Multiply()([x_latent_gaussian_input, x_latent_stddev])
    # x = tf.keras.layers.Add()([x, x_latent_mean])
    x = tf.keras.layers.Add()([x_latent_gaussian_input, x_latent_mean])
    ...
    for i in range(output_layers):
        x_mean = tf.keras.layers.Conv2D(filters=x_mean.shape[-1],
                                        kernel_size=(3, 3),
                                        strides=(1, 1),
                                        padding="same",
                                        dilation_rate=1,
                                        kernel_initializer=tf.keras.initializers.he_uniform)(x_mean)
    ...
    for i in range(output_layers):
        x_stddev = tf.keras.layers.Conv2D(filters=x_stddev.shape[-1],
                                          kernel_size=(3, 3),
                                          strides=(1, 1),
                                          padding="same",
                                          dilation_rate=1,
                                          kernel_initializer=tf.keras.initializers.he_uniform)(x_stddev)
    ...
    x_stddev = tf.keras.layers.Conv2D(filters=image_input_shape[-1],
                                      ...
                                      padding="same",
                                      dilation_rate=1,
                                      kernel_initializer=tf.keras.initializers.zeros)(x_stddev)
    x_stddev = tf.keras.layers.Lambda(tf.keras.activations.softplus)(x_stddev)
    x_stddev = tf.keras.layers.Lambda(lambda current_x: current_x + 0.3068528)(x_stddev)
...
def validate(model, x_test_images, standard_scaler, x_test_labels, i):
    ...
    if conv_bool and alex_bool:
        if i + 1 > mean_squared_error_epoch and i + 1 > root_mean_squared_error_epoch:
            y_pred_mean, y_pred_stddev, y_latent_mean, y_latent_stddev = (
                model([current_x_test_image, current_x_test_label, tf.random.normal((1,) + latent_gaussian_shape)],
                      training=False))

            y_pred_means = [y_pred_mean]
            y_pred_stddevs = [y_pred_stddev]

            for j in range(int(tf.math.ceil(tf.math.reduce_max(y_latent_stddev) * 32.0)) - 1):
                y_pred_mean, y_pred_stddev, y_latent_mean, y_latent_stddev = (
                    model([current_x_test_image, current_x_test_label, tf.random.normal((1,) + latent_gaussian_shape)],
                          training=False))
            ...
            y_pred_mean, y_pred_stddev, y_latent_mean, y_latent_stddev = (
                model([current_x_test_image, current_x_test_label, tf.zeros((1,) + latent_gaussian_shape)],
                      training=False))
    ...
    output_image(current_y_pred, standard_scaler, "{0}/{1}.png".format(validate_output_path, str(i + 1)))
...
def train_gradient_accumulation(model, optimiser, batch_sizes, batch_sizes_epochs, x_train_images, x_test_images,
    ...
    for i in range(epochs):
        ...
        for j in range(iterations):
            ...
            for m in range(current_batch_size):
                ...
                if conv_bool and alex_bool:
                    if i + 1 > mean_squared_error_epoch:
                        if i + 1 > root_mean_squared_error_epoch:
                            with (tf.GradientTape() as tape):
                                y_pred_mean, y_pred_stddev, y_latent_mean, y_latent_stddev = (
                                    model([current_x_train_image, current_x_train_label,
                                           tf.random.normal((1,) + latent_gaussian_shape)], training=True))

                                loss = tf.math.reduce_sum([
                                    gaussian_negative_log_likelihood(current_x_train_image, y_pred_mean, y_pred_stddev),
                                    latent_loss_weight * gaussian_kullback_leibler_divergence(y_latent_mean,
                                                                                              y_latent_stddev)])
                            ...
                            with (tf.GradientTape() as tape):
                                y_pred_mean, y_pred_stddev, y_latent_mean, y_latent_stddev = (
                                    model([current_x_train_image, current_x_train_label,
                                           tf.zeros((1,) + latent_gaussian_shape)], training=True))

                                loss = tf.math.reduce_sum([
                                    root_mean_squared_error(current_x_train_image, y_pred_mean),
                                    root_mean_squared_error(tf.ones(y_pred_stddev.shape), y_pred_stddev),
                                    root_mean_squared_error(tf.ones(y_latent_stddev.shape), y_latent_stddev)])
                        ...
                        with (tf.GradientTape() as tape):
                            ...
                            loss = tf.math.reduce_sum([
                                mean_squared_error(current_x_train_image, y_pred_mean),
                                mean_squared_error(tf.ones(y_pred_stddev.shape), y_pred_stddev),
                                mean_squared_error(tf.ones(y_latent_stddev.shape), y_latent_stddev)])
                    ...
                    if i + 1 > mean_squared_error_epoch:
                        with tf.GradientTape() as tape:
                            ...
                            loss = root_mean_squared_error(current_x_train_image, y_pred)
                    else:
                        with tf.GradientTape() as tape:
                            y_pred = model([current_x_train_image], training=True)

                            loss = mean_squared_error(current_x_train_image, y_pred)
            ...
            if i + 1 > mean_squared_error_epoch:
                if conv_bool and alex_bool and i + 1 > root_mean_squared_error_epoch:
        ...
        validate(model, x_test_images, standard_scaler, x_test_labels, i)

    return model


def train(model, optimiser, batch_sizes, batch_sizes_epochs, x_train_images, x_test_images, standard_scaler,
            ...
            if conv_bool and alex_bool:
                if i + 1 > mean_squared_error_epoch:
                    if i + 1 > root_mean_squared_error_epoch:
                        with tf.GradientTape() as tape:
                            y_pred_mean, y_pred_stddev, y_latent_mean, y_latent_stddev = (
                                model([current_x_train_images, current_x_train_labels,
                                       tf.random.normal((current_batch_size,) + latent_gaussian_shape)], training=True))

                            loss = tf.math.reduce_sum([
                                gaussian_negative_log_likelihood(current_x_train_images, y_pred_mean, y_pred_stddev),
                                latent_loss_weight * gaussian_kullback_leibler_divergence(y_latent_mean,
                                                                                          y_latent_stddev)])
                        ...
                        with tf.GradientTape() as tape:
                            y_pred_mean, y_pred_stddev, y_latent_mean, y_latent_stddev = (
                                model([current_x_train_images, current_x_train_labels,
                                       tf.zeros((current_batch_size,) + latent_gaussian_shape)], training=True))

                            loss = tf.math.reduce_sum([
                                root_mean_squared_error(current_x_train_images, y_pred_mean),
                                root_mean_squared_error(tf.ones(y_pred_stddev.shape), y_pred_stddev),
                                root_mean_squared_error(tf.ones(y_latent_stddev.shape), y_latent_stddev)])
                    ...
                    with tf.GradientTape() as tape:
                        ...
                        loss = tf.math.reduce_sum([
                            mean_squared_error(current_x_train_images, y_pred_mean),
                            mean_squared_error(tf.ones(y_pred_stddev.shape), y_pred_stddev)])#,
                            # mean_squared_error(tf.ones(y_latent_stddev.shape), y_latent_stddev)])
                ...
                if i + 1 > mean_squared_error_epoch:
                    with tf.GradientTape() as tape:
                        ...
                        loss = root_mean_squared_error(current_x_train_images, y_pred)
                else:
                    with tf.GradientTape() as tape:
                        y_pred = model([current_x_train_images], training=True)

                        loss = mean_squared_error(current_x_train_images, y_pred)
...
def test(model, x_test_images, standard_scaler, x_test_labels):
        ...
        if conv_bool and alex_bool:
            y_pred_mean, y_pred_stddev, y_latent_mean, y_latent_stddev = (
                model([current_x_test_images, current_x_test_labels,
                       tf.random.normal((test_batch_size,) + latent_gaussian_shape)], training=False))
            ...
            for j in range(int(tf.math.ceil(tf.math.reduce_max(y_latent_stddev) * 32.0)) - 1):
                y_pred_mean, y_pred_stddev, y_latent_mean, y_latent_stddev = (
                    model([current_x_test_images, current_x_test_labels,
                           tf.random.normal((1,) + latent_gaussian_shape)], training=False))
    ...
    print("Error: {0:12}%".format(str(error.numpy())))
...
def main():
    ...
    x_train_images, x_test_images, standard_scaler, x_train_labels, x_test_labels = (
    ...
    if gradient_accumulation_bool:
        ...
        model = train(model, optimiser, batch_sizes, batch_sizes_epochs, x_train_images, x_test_images, standard_scaler,
                      x_train_labels, x_test_labels)
